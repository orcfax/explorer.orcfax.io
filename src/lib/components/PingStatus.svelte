<script lang="ts">
	...
	let { color, size = 'sm' }: Props = $props();
</script>

<span class={`relative flex ${size === 'sm' ? 'h-2 w-2' : 'h-[10px] w-[10px'}`}>
	<span
	...
	></span>
	<span
		class={`relative inline-flex rounded-full ${size === 'sm' ? 'h-2 w-2' : 'h-[10px] w-[10px]'}`}
